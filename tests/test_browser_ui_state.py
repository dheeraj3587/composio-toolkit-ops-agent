...
to be in ``browser_running``, and never because the provider recorded on the run
row implies a session that could be driven.
"""
...
# --- configuration is judged by what this backend actually needs --------------
def test_a_configured_browser_service_is_configured() -> None:
...
def test_the_live_opt_in_alone_does_not_configure_the_browser() -> None:
    ...
    # There is one backend, so an unnamed provider resolves to it rather than
    # reporting a second one that could have been configured instead.
    assert state.provider == "playwright"
...
def test_a_legacy_provider_row_is_never_treated_as_a_drivable_hosted_session() -> None:
    """A run recorded under the retired cloud adapter reports it, and shows nothing.

    The provider is named as recorded — the row is what happened. But no hosted
    session exists behind that name any more, so the view must not become a
    drivable embed pointing at a backend that cannot answer.
    """
...
def test_credential_submission_still_requires_the_owner_policy_opt_in() -> None:
    without_opt_in = _PLAYWRIGHT.model_copy(update={"allow_local_credential_submission": False})
...
def test_plan_only_runs_report_no_browser_attempt() -> None:
    state = project_browser_ui(
        settings=_PLAYWRIGHT, run_status="route_selected", event_types=set(), plan_only=True
    )
...
def test_production_api_main_projection_is_provider_aware(tmp_path: Path) -> None:
    ...
    # Readiness is reported for the one backend that exists, and no retired
    # provider is still advertised as a capability the deployment has.
