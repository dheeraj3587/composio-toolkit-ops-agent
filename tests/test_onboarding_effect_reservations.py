...
submission's reservation, then replay that exact boundary. The point of the test
is the atomicity claim — both rows land in one unit of work — and the replay
claim: a replayed boundary reserves nothing new and hands the worker a
disposition that does not authorize a second submission.
"""
...
def test_phase_and_reservation_land_together_and_a_replay_reserves_nothing_new(
    ...
    with sqlite3.connect(db_path) as connection:
        assert connection.execute(
            "SELECT status FROM side_effect_intents WHERE run_id = ? AND operation_key = ?",
            ("run-001", KEY),
        ).fetchone() == ("pending",)
    assert store.submission_count(run_id="run-001", operation_key=KEY) == 1

    history_before = store.history(run_id="run-001")
    replayed = _reserve_signup(store)

    # A replayed boundary writes no history row and reserves nothing new; the key
    # it reports is the one already held, and it is not executable.
    assert replayed.committed is False
    assert replayed.plan.disposition == "reconcile"
    ...
    # An ambiguous outcome leaves the submission count where it is and authorizes
    # nothing further for the key.
