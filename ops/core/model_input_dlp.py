...
# The one reason code for a refused projection. It already exists in the closed
# onboarding vocabulary; this module adds no new code to it.
DLP_REFUSAL_REASON_CODE: Final[OnboardingReasonCode] = "dlp_prompt_refused"


@dataclass(frozen=True, slots=True)
class ModelInputDecision:
    """Whether one assembled projection may cross to an inference backend.

    A refusal is a value, not an exception: the action loop turns it into a
    discard-and-reobserve iteration rather than aborting the run, so this type is
    something a caller branches on. ``prompt`` is empty on a refusal so a refused
    projection cannot be forwarded by accident, and ``reason_code`` is set only on
    a refusal (an admitted projection has nothing to report).
    """
...
def screen_model_input(prompt: str) -> ModelInputDecision:
    """Admit or refuse a fully sanitized projection at the inference boundary.

    Belt and braces: every field in ``prompt`` should already have passed through
    the sanitizers above, so tripping :func:`contains_secret_material` here means a
    channel escaped them. The projection is then refused rather than sent, carrying
    :data:`DLP_REFUSAL_REASON_CODE`, and the caller discards it and observes again.
    """

    if contains_secret_material(prompt):
        return ModelInputDecision(allowed=False, prompt="", reason_code=DLP_REFUSAL_REASON_CODE)
    return ModelInputDecision(allowed=True, prompt=prompt, reason_code=None)
