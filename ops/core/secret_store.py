...
# The lifecycle of a stored credential row. ``active`` is every row this vault has
# ever written; the other two are the two ways an onboarding run abandons a
# captured credential (design LL-5.3, Requirements 10.17, 10.19).
#
# WHY A STATUS AND NOT A DELETE. An abandoned credential is an audit question —
# "which credential did we abandon, and why" — and a deleted row cannot answer it.
# A retained row can, and retaining it costs nothing in exposure because
# ``get`` refuses to resolve a row that is not ``active``: the ciphertext stays
# for the audit while the value stops being readable through any reference.
CredentialLifecycleStatus = Literal["active", "superseded", "unusable"]
CREDENTIAL_LIFECYCLE_STATUSES: frozenset[str] = frozenset({"active", "superseded", "unusable"})
_ACTIVE_CREDENTIAL_STATUS = "active"
...
class SQLiteSecretStore:
    ...
    def initialize(self) -> None:
        with self._connect() as connection:
            ...
            for column, ddl in (
                ...
                # The credential lifecycle column of design LL-5.3. Additive with
                # a default, so every row this vault already holds is ``active``
                # and no existing writer or reader changes behaviour.
                (
                    "credential_status",
                    "ALTER TABLE vault_entries ADD COLUMN credential_status "
    ...
    def account_login_references(self, *, app_slug: str, account_ref: str) -> dict[str, str]:
        """Reference-only presence probe for one bound account's login pair.

        The vault-first admission decision (``ops.onboarding.admission``) has to
        answer "does a login exist for this app and this account binding" without
        any plaintext entering the deciding process. This read is that answer: the
        query selects ``kind`` and ``id`` and never ``ciphertext``, so there is
        nothing here to decrypt — the absence of a value is structural rather than
        a discipline the caller has to keep.

        Returns ``{field: reference}`` for whichever reusable login fields are
        stored, so a partial pair is reported as a partial pair; deciding what an
        incomplete pair means belongs to the caller, not to the vault. An unknown
        app slug or account binding reads as ``{}``, matching the sibling readers:
        "nothing stored" is a normal autonomous-run condition, not an error.
        """

        if _APP_SLUG.fullmatch(app_slug) is None or _ACCOUNT_REF.fullmatch(account_ref) is None:
            return {}
        kinds = {_account_login_kind(field): field for field in sorted(REUSABLE_LOGIN_FIELDS)}
        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT kind, id
        ...
        for kind, identifier in rows:
            field = kinds.get(str(kind))
            if field is None or field in references:
                continue
            references[field] = f"vault://{app_slug}/{kind}/{identifier}"
        return references
    ...
    def get(self, reference: str) -> str:
        ...
        if row is None:
            # A superseded or unusable row is indistinguishable from an absent one
            # here on purpose: the row survives for the audit, and the value it
            # holds stops being readable through any reference the moment the run
            # abandons it (Requirements 10.17, 10.19).
    ...
    def mark_credential_superseded(self, reference: str) -> str:
        """Retire a credential the run replaced with a freshly minted one.

        The supersede half of the credential ladder (Requirement 10.17): the
        provider called this credential ``invalid``, the run is about to mint
        another under a new generation, and this row must stop resolving without
        losing the audit answer to "which credential did we abandon".

        POST: the row's ``credential_status`` is ``superseded``, the ciphertext is
              untouched, no row is deleted, and :meth:`get` no longer resolves the
              reference. Idempotent: re-marking an already superseded row is a
              no-op so a replayed supersede does not fail the phase.
        """

        return self._set_credential_status(reference, status="superseded")

    def mark_credential_unusable(self, reference: str) -> str:
        """Retire a credential the run will not replace (Requirement 10.19).

        The terminal half of the same ladder: the generation budget is spent, so
        the reference is marked unusable, left unpublished, and kept for the
        audit. Same guarantees as :meth:`mark_credential_superseded`, including
        idempotency.
        """

        return self._set_credential_status(reference, status="unusable")

    def _set_credential_status(self, reference: str, *, status: str) -> str:
        """Move one durable row's lifecycle status. Never deletes (design LL-5.3).

        The ``credential_status IN ('active', :status)`` predicate is what makes
        the write idempotent *and* one-directional: an ``active`` row moves, a row
        already at the target status is rewritten to the same value, and a row at
        the *other* terminal status matches nothing and is reported as absent
        rather than being quietly resurrected into a different verdict.
        """

        if status not in CREDENTIAL_LIFECYCLE_STATUSES or status == _ACTIVE_CREDENTIAL_STATUS:
            raise ValueError("a credential row is moved to a retired lifecycle status")
        app_slug, kind, identifier = self._parse_reference(reference)
        with self._connect() as connection:
            cursor = connection.execute(
                """
                ...
                WHERE id = ? AND app_slug = ? AND kind = ? AND one_time = 0
                  AND credential_status IN ('active', ?)
                """,
                (status, identifier, app_slug, kind, status),
            )
        if cursor.rowcount != 1:
            raise SecretNotFoundError("secret reference was not found")
        return status
