...
container HEALTHCHECK entrypoint (``python -m ops.browser.readiness``) and the
...
from ops.browser.process_hardening import harden_playwright_parent_process
...
async def probe_playwright(*, timeout_seconds: float = 30.0) -> BrowserReadiness:
    ...
    # real sessions. It must use the identical strict child environment and the
    # identical launch flags rather than inheriting broker, storage, RPC, or
    # provider credentials merely because it is short-lived. Sharing the flag
    # builder is what keeps the probe honest: a probe that launched differently
    # from a real session could stay green while every session failed.
    ...
    args = chromium_launch_args(headless=not interactive, sandbox_disabled=sandbox_disabled)
