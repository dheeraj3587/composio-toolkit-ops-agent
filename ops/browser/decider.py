...
/ ``build_choice_prompt``, driven by ``ops.browser.candidates``. The model returns
...
class BrowserAction(BaseModel):
    ...
    Superseded by ``CandidateChoice`` + ``ops.browser.candidates``: the model may no
...
def build_choice_prompt(
    *,
    ...
    caller through ops.core.model_input_dlp; this function never sanitizes on its own so
