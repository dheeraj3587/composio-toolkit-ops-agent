      ...
      {phaseBlueprint.map(({ key, name, icon: Icon, copy }) => {
        ...
        return (
          <article key={key} className="flex min-h-44 flex-col justify-between bg-card p-4 [&:not(:last-child)]:border-b [&:not(:last-child)]:border-border sm:[&:not(:last-child)]:border-b-0 sm:[&:not(:last-child)]:border-r">
            ...
            <div>
              <p className="font-mono text-2xs uppercase tracking-[0.12em] text-muted-foreground">{phase ? "Backend reported" : "Not reported"}</p>
    ...
    <Card className="h-full rounded-lg border-border bg-card py-0 shadow-none">
      <CardHeader className="border-b border-border px-5 py-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Badge variant="outline" className="rounded-md font-mono text-2xs uppercase tracking-[0.1em]">{Math.round(research.confidence * 100)}% confidence</Badge>
        </div>
      </CardHeader>
  ...
  return (
    <Card className="h-full rounded-lg border-white/10 bg-rail py-0 text-white shadow-none">
      <CardHeader className="border-b border-white/10 px-5 py-4">
        <p className="font-mono text-2xs uppercase tracking-[0.14em] text-brand-300">Security boundary</p>
      ...
      <CardContent className="space-y-3 px-5 py-5">
        {safeguards.map(({ label, value, icon: Icon }) => (
          <div key={label} className="flex items-center justify-between gap-4 border-b border-white/10 pb-3 last:border-0 last:pb-0">
            ...
            <span className="text-right font-mono text-2xs uppercase tracking-[0.1em] text-white/85">{controlValue(value)}</span>
          </div>
        ))}
  ...
  return (
    <Card className="h-full rounded-lg border-amber-300 bg-amber-50 py-0 shadow-none dark:border-amber-500/35 dark:bg-amber-500/10">
      <CardContent className="flex min-h-52 flex-col justify-between px-5 py-5">
        ...
        <div>
          <p className="font-mono text-2xs uppercase tracking-[0.12em] text-amber-800 dark:text-amber-300">Human action · {humanize(request.action_type)}</p>
          ...
          <p className="mt-3 font-mono text-2xs uppercase tracking-[0.1em] text-amber-800/80 dark:text-amber-300/80">
  ...
  return (
    <Card className="h-full rounded-lg border-emerald-300 bg-emerald-50/60 py-0 shadow-none dark:border-emerald-500/35 dark:bg-emerald-500/10">
      <CardHeader className="border-b border-emerald-200 dark:border-emerald-500/25 px-5 py-4">
        <p className="font-mono text-2xs uppercase tracking-[0.13em] text-emerald-700 dark:text-emerald-300">Output · references only</p>
  ...
  return (
    <Card className="h-full rounded-lg border-border bg-card py-0 shadow-none">
      ...
      <CardContent className="grid gap-5 px-5 py-5 sm:grid-cols-2">
        ...
        <p className="sm:col-span-2 border-t border-border pt-4 font-mono text-2xs uppercase tracking-[0.1em] text-muted-foreground">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {profile.allowed_host_patterns.length
              ? profile.allowed_host_patterns.map((pattern) => (
                  <Badge key={pattern} variant="outline" className="rounded-md font-mono text-2xs">{pattern}</Badge>
                ))
            ...
            {profile.auxiliary_hosts.length
              ? profile.auxiliary_hosts.map((host) => (
                  <li key={`${host.kind}:${host.host}`} className="flex flex-wrap items-center gap-2">
                    <span className="font-mono text-2xs">{host.host}</span>
                    <Badge variant="outline" className="rounded-md font-mono text-2xs uppercase tracking-[0.1em]">{humanize(host.kind)}</Badge>
                  </li>
                ))
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap justify-end gap-2">
          ...
          {flow.requires_approval ? (
            <Badge variant="outline" className="rounded-md font-mono text-2xs uppercase tracking-[0.1em]">Approval required</Badge>
          ) : null}
          {flow.requires_billing ? (
            <Badge variant="outline" className="rounded-md font-mono text-2xs uppercase tracking-[0.1em]">Billing required</Badge>
          ) : null}
        </div>
      </div>
...
function EvidenceRow({ evidence }: { evidence: FieldEvidenceView }) {
  return (
    <li className="rounded-md border border-border p-3">
      ...
      <p className="mt-1 break-words font-mono text-2xs">{evidence.value}</p>
